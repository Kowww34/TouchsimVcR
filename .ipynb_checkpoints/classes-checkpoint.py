...
#for random afferents
from .constants import SA_pca, RA_pca, PC_pca, SA_scaler, RA_scaler, PC_scaler
    ...
    def __init__(self,affclass,**args):
        ...
        Kwargs:
            ...
            depth (float) = Depth of afferent in the skin (default: standard depth
                depening on afferent class).
            idx (int): ID number of neuron model (default: randomly chosen).
            sigma_scalar (float): scales the randomness of the afferent parameters
        """
        self.affclass = affclass
        self.location = np.atleast_2d(args.get('location',np.array([[0., 0.]])))
        self.depth = args.get('depth',None)
        ...
        # Set afferent parameters using fit data
        random_afferent_parameters = np.zeros((1,13))
        
        if affclass == 'SA1':
            fuzzy_point = np.column_stack([np.random.normal(0, self.C*constants.SA_sigma_x, 1),
                                        np.random.normal(0, self.C*constants.SA_sigma_y, 1),
            ...
            fuzzy_point = np.column_stack([
                np.random.normal(0, self.C * constants.RA_sigma_x, 1),
                np.random.normal(0, self.C * constants.RA_sigma_y, 1),
            ...
            fuzzy_point = np.column_stack([
                np.random.normal(0, self.C * constants.PC_sigma_x, 1),
                np.random.normal(0, self.C * constants.PC_sigma_y, 1),
                *[np.zeros((1,)) for _ in range(2)]])
