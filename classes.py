...
class Stimulus(object):
    ...
    @property
    def time(self):
        return np.arange(self.trace.shape[1]) / self.fs
    ...
    def strain(self, region="D2", surface=hand_surface,
        ...
        """Return the stress (static and/or dynamic) that an afferent would feel
        at the center of a specified surface region, with propagation (delay and
        decay for dynamic) from all stimulus pins.

        Args:
            region (str or int): Region tag (e.g. 'D3d_t') or region index.
            component (str): 'static', 'dynamic', or 'both' (default: 'both').
            surface (Surface object): Surface used to get region center
                (default: hand_surface).
            affclass (str): Afferent class for nominal depth
                (default: 'PC'). One of 'SA1', 'RA', 'PC'.
            depth (float): Depth in mm at which to compute stress. If None,
                uses the standard depth for affclass.

        Returns:
            If component == 'both', dict with keys 'time', 'static', 'dynamic',
            each stress array shape (n_samples,). Otherwise (time, stress) with
            stress shape (n_samples,).
        """
        if region == "D2":
            center = hand_surface.centers[hand_surface.tag2idx("D2d_t")]
        elif region == "D3":
            center = hand_surface.centers[hand_surface.tag2idx("D3d_t")]
        elif region == "D4":
            center = hand_surface.centers[hand_surface.tag2idx("D4d_t")]
        elif region == "D5":
            center = hand_surface.centers[hand_surface.tag2idx("D5d_t")]
        else:
            raise ValueError("region must be D2, D3, D4, or D5")

        aff = Afferent(affclass, location=center)
        static, dynamic, _ = self.propagate(aff)
        static = np.squeeze(static)
        dynamic = np.squeeze(dynamic)

        return self.time, static, dynamic

    def pressure(self, region="D2", surface=hand_surface, agg="mean",
        component="static", empty_value=0.0):
        """Returns pressure profiles per surface region as a function of time.

        Args:
            surface (Surface object): Surface used to map pin locations to
                regions (default: hand_surface).
            regions (str|int|iterable): Region tag(s) or region id(s) to include.
                None selects all surface regions.
            agg (str|callable): Aggregation across pins in a region. Supported
                strings: 'sum', 'mean', 'max', 'min', 'median'.
            component (str): 'static', 'dynamic', or 'both'.
            empty_value (float): Value used for regions with no pins.

        Returns:
            If component == 'both', returns a dict with keys 'static' and
            'dynamic', each mapping to (time, profiles, tags). Otherwise returns
            (time, profiles, tags), where profiles has shape (R, T).
        """
        surface = hand_surface
        if region == "D2":
            rid = surface.tag2idx("D2d_t")
        elif region == "D3":
            rid = surface.tag2idx("D3d_t")
        elif region == "D4":
            rid = surface.tag2idx("D4d_t")
        elif region == "D5":
            rid = surface.tag2idx("D5d_t")
        else:
            raise ValueError("region must be D2, D3, D4, or D5")

        #profiles
        static_prof = self._profile              #(T, pins)
        dynamic_prof = self._profiledyn.T        #(T, pins)

        #which pins belong to region
        _, region_ids = surface.locate(self.location)
        mask = (region_ids == rid)

        if not np.any(mask):
            T = static_prof.shape[0]
            zero = np.zeros(T)
            return self.time, zero, zero

        static = np.mean(static_prof[:, mask], axis=1)
        dynamic = np.mean(dynamic_prof[:, mask], axis=1)
...
class Response(object):
    ...
    @property        
    def total(self):
        return int(np.sum(self.rate())*self.duration)
    @property        
    def SAtotal(self):
        return int(np.sum(self.rate()[self.aff.find('SA1')])*self.duration)
    @property        
    def RAtotal(self):
        return int(np.sum(self.rate()[self.aff.find('RA')])*self.duration)
    @property        
    def PCtotal(self):
        return int(np.sum(self.rate()[self.aff.find('PC')])*self.duration) 
