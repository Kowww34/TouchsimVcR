...
def plot_stimulus(obj,**args):
    ...
    if not spatial:
        ...
        if not grid:
            hvobj = hvobj.options(xlabel="Time (s)", ylabel="z (mm)")
        ...
        for t in range(num):
            p = hv.Polygons([{('x','y'):hv.Ellipse(locs[l,0],locs[l,1],2*rad).array(),
                'z':d[l,t]} for l in range(obj.location.shape[0])],vdims='z')
            p = hv.util.opts.apply_groups(
                p,
                plot=dict(color ='z',aspect='equal'),
                style=dict(linewidth=0.,line_width=0.01))
            hm[t] = p
...
def plot_response(obj,**args):
    ...
    if not spatial:
        ...
        for i in range(len(idx)):
            ...
            spikes[i] = hv.opts.apply_groups(s, plot=dict(position=0.1*i,spike_length=0.1),
                style=dict(color=tuple(
                Afferent.affcol[obj.aff.afferents[idx[i]].affclass])))
        hvobj = hv.NdOverlay(spikes)
        ...
        for t in range(r.shape[1]):
            ...
            for a in Afferent.affclasses:
                ...
                if scale:
                    points[a] = hv.opts.apply_groups(p,
                                                     style=dict(color=tuple(Afferent.affcol[a])),
                                                     plot=dict(size = 2,scaling_factor=scaling_factor, aspect='equal')
                                                    )
                else:
                    points[a] = hv.opts.apply_groups(p,
                                                     plot=dict(color_index=2,aspect='equal')
                                                    ).options(cmap='fire_r')
...
def plot_surface(obj,**args):
    ...
    if np.min(wh)<250:
        wh = wh/np.min(wh)*250
    hvobj = hv.Path([obj.boundary[i] for i in idx])
    hvobj = hv.util.opts.apply_groups(hvobj,
                                    style=dict(color='k'),
                                    plot=dict(yaxis=None, xaxis=None, aspect='equal', width=int(ceil(wh[0])), height=int(ceil(wh[1])))
                                     )
